...
import { AssignRequestOrderModel } from 'src/app/modules/orders/_models/assign-order.model';
...
export class BaseSignalService {
  ...
  constructor(protected sessionService: SessionService) {
    this.serverURL = environment.apiUrl.replace('api', '');
  }
  ...
  assignOrder(order: AssignRequestOrderModel) {
    this.checkConnectionState();

    if (this.isConnected) {
      this.hubConnection.invoke('GetAssignedOrder', order);
      // .then((res) => console.log('RESULT ' + res))
      // .catch((err) => console.log('CATCH' + err));
    }
  }
